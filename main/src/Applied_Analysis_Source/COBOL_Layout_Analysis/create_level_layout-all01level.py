...
def COPY展開命令行(行情報,include_copy_line_flag):

    if 行情報.startswith("COPY01*") and include_copy_line_flag:
        return True
    else:
        return False
...
def スキーマ明細行出力(tmp_layout):
    ...
    OutPut_Path = os.path.join(OutPut_folder,スキーマ名)
...
def WORK_AREA_レイアウト情報出力処理(TmpSheet,include_copy_line_flag):
        ...
        while 検索行 < len(TmpSheet):
            ...
            if 有効行(対象行文字列):
                ...
                #    Set TSO = myFSO.CreateTextFile(Filename:=スキーマフォルダ + "\\" + スキーマ名, Overwrite:=True)
                    chk_end = False
                    if include_copy_line_flag:
                        検索行_copy = 検索行-1
                        while 検索行_copy >= 0:
                            対象行文字列_copy = TmpSheet[検索行_copy]
                            if 有効行(対象行文字列_copy):
                                break
                            if COPY展開命令行(対象行文字列_copy,include_copy_line_flag):
                                tmp_layout.append(対象行文字列_copy)
                            検索行_copy -= 1
                    while True:
                        if 有効行(対象行文字列):
                            if (" FD " in string_7_66) or (" SD " in string_7_66) : # 'QA管理表の0011管理番号を対応 (李)
                                # 'スキーマ情報終了
                                chk_end = True
                                ...
                                chk_end = True
                            elif " EJECT " in string_7_66:
                                pass
                            else:
                                tmp_layout.append(対象行文字列)
                                # Call ②_スキーマ明細行出力
                        else:
                            if COPY展開命令行(対象行文字列,include_copy_line_flag):
                                tmp_layout.append(対象行文字列)  
                        検索行 = 検索行 + 1
                        chk_1st = False
...
def スキーマファイル出力(TmpSheet,include_copy_line_flag):
    ...
    while 検索行 < len(TmpSheet):
        ...
        if (" FD " in string_7_66 or " SD " in string_7_66) and \
            ...
            while 検索行 < len(TmpSheet):
                ...
                elif (" FD " in string_7_66) or (" SD " in string_7_66) :  #'QA管理表の0011管理番号を対応 (李)
                    break
                elif " SECTION" in string_7_66:
                    break
                ...
                while True:
                    if 有効行(対象行文字列):
                        ...
                        # 'elif " 01 " in string_7_66 and \ot (chk_1st):
                            ...
                            if tmp_layout and COPY展開命令行(tmp_layout[-1],include_copy_line_flag):
                                line_before = tmp_layout.pop()

                            結果一覧出力()
                            スキーマ明細行出力(tmp_layout)
                            tmp_layout = []

                            if COPY展開命令行(line_before,include_copy_line_flag):
                                tmp_layout.append(line_before)
...
def スキーマファイル出力_01_All(TmpSheet,include_copy_line_flag):
    ...
    while 検索行 < len(TmpSheet):
        ...
        ## PROCEDURE DIVISION に入ったら レイアウトはなしとして break
        if " PROCEDURE " in string_7_66:
            break
        
        
        # 01レベルではないならSkip
        if not (" 01 " in Mid(対象行文字列, 6, 6) or  " 1 " in Mid(対象行文字列, 6, 6)):
            検索行 += 1
            continue
        ...
        SCM_開始行_list = ArrayEmptyDelete(string_7_66.split(" "))
        
        if len(SCM_開始行_list) < 2:
            
            print("Error: 01 開始行の誤検知 or ソースコードに不備があります。 該当行 = ",対象行文字列)
            
            検索行 += 1
            continue
        
        
        レコード名 = SCM_開始行_list[1].replace("01", "").replace(" ", "").replace(".", "")
        DD名 = レコード名
        chk_1st = True
        chk_end = False
        スキーマ名 = モジュール名 + "_" + DD名 + "_" + str(ファイル連番).zfill(2) + ".scm"
...
def main(db_path, Folder_COBOL_path, Output_folder_path,include_copy_line_flag=True):
    ...
    for file_path in COBOL_Files:
        ...
        # モジュール名 = take_extensions(cobol_file)
        cobol_file,_,_,モジュール名 = GetFileInfo(cobol_file)
       
        
        TmpSheet = TEXT_READ処理(file_path)
        ファイル連番 = 1
        スキーマファイル出力_01_All(TmpSheet,include_copy_line_flag)
        結果一覧出力()

...
class 利用DSN一覧:
    ...
    def setup(self):
        ...
        for dsn,dsn_group in zip(df["DSN名"],df["データセットグループ①"]):
            if dsn_group not in self.dic:
                self.dic[dsn_group] = set()
            self.dic[dsn_group].add(dsn)
...
def マルチレイアウト解析明細処理(myRS2,OutSheet_GYO,解析単位):
    global スキーマ_基本情報_,レイアウト解析文字列_,利用DSN一覧_
    global OutSheet2,マルチレイアウト判定
    TmpSheet = []
 
    dsn_set = 利用DSN一覧_.get(解析単位)
    used_dsn_set = set()
    for data in myRS2:
        OutSheet2_GYO = [""]*11
    ...
    for dsn in dsn_set:
        if dsn in used_dsn_set:
            continue
               
        OutSheet2_GYO = [""]*11
        
        OutSheet2_GYO[1] = 解析単位
        OutSheet2_GYO[2] = dsn
        OutSheet2_GYO[3] = "DSN対応レイアウトなし"
        OutSheet2.append(OutSheet2_GYO)
        
    return TmpSheet,OutSheet_GYO

        
def muiti_layout_main(db_path,title,マルチレイアウトチェック単位="①データセットグループ",multicheck_folder=None):
    ...
    for i in range(len(df)):
        ...
        TmpSheet,OutSheet_GYO = マルチレイアウト解析明細処理(myRS2,OutSheet_GYO,解析単位)

        if multicheck_folder != None and len(TmpSheet) > 1:
            multicheck_out_path = os.path.join(multicheck_folder,PARM + "_マルチCHK.txt")
            with open(multicheck_out_path,"w") as f:
                for line in TmpSheet:
                    f.write(line + "\n")

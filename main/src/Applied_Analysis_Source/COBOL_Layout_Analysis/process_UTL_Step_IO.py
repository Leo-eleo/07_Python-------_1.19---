...
def process_UTL_Step_IO_main(db_path,title):
    ...
    sql = "SELECT * FROM UTL_STEP別_IO情報_DSNテーブル"
    
    df = pd.read_sql(sql,conn)
    step_io_table_keys = df.columns.tolist()
    step_io_table_keys.remove("補足")
    sql = "SELECT * FROM QRY_UNION_IO情報テーブル"
    
    df = pd.read_sql(sql,conn)
    df.fillna("",inplace=True)
    qry_union_io_table_keys = df.columns.tolist()
    sql,_ = make_delete_sql("UTL_STEP別_IO情報_DSNテーブル",[],[])
    ...
    for i in range(len(df)):
        data = df.iloc[i]
        value =[i+1] + [data[key] for key in qry_union_io_table_keys]
        sql,values = make_insert_sql("UTL_STEP別_IO情報_DSNテーブル",value,step_io_table_keys)
    ...
    for i in range(len(ActSheet)):
        ...
        if ActSheet[i][13] == "":
            ...
            # '通常パターン
                ...
                ActSheet[i][13] = "OK"
                identify_resolution_set.add((ActSheet[i][9],ActSheet[i][12]))
    ...
    output_header = ["JCL_NAME","JOB_SEQ","JOB_ID","STEP_SEQ","STEP名","Utility_ID","INPUT_DD名","INPUT","INPUT_DSN","OUTPUT_DD名","OUTPUT","OUTPUT_DSN","除外STEP"]

        ...
        if COBOL階層情報 == "環境部":
            ...
            if CMD_fld == "READ" and len(
                ...
                if レコードID有無判定_COBOL(L_MOVE元, L_MOVE元, L_varArray):
                    ...
                    SUB_SQL生成_COBOL_5_.insert()
        # 20230715
        # Natural Read and Write Work
        elif COBOL階層情報 == "":
            key_index = 6
            if TokenSheet2_GYO[key_index].upper() == "READ" or TokenSheet2_GYO[key_index].upper() == "WRITE":
                if TokenSheet2_GYO[key_index + 1].upper() == "WORK":
                    io_way = "INPUT" if TokenSheet2_GYO[key_index].upper() == "READ" else "OUTPUT"
                    io_target = TokenSheet2_GYO[key_index + 2] if TokenSheet2_GYO[key_index + 2].upper() != "FILE" else TokenSheet2_GYO[key_index + 3]
                    io_target = int(io_target)
                    io_target_f = f"CMWKF{io_target}" if io_target >= 10 else f"CMWKF0{io_target}"
                    # 共通_PGM_IO情報
                    SUB_SQL生成_共通_3_.insert("NATURAL", ライブラリID, COBOL_ID, io_way, io_target_f)
                    # ②COBOL_入出力情報1
                    検索行文字列 = 検索行文字列生成処理(TokenSheet2_GYO[基準列:])  #'処理開始列（5列目を引数にする）
                    SELECT_LINE_INFO = 検索行文字列
                    SELECT_ID = io_target_f
                    DD_ID = SELECT_ID
                    SUB_SQL生成_COBOL_1_.insert()

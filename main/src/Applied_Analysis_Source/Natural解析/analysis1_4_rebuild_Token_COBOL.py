...
def analysis1_4_rebuild_Token_COBOL(TokenSheet):
    ...
    # PERFORM 内部サブルーチン CHECK
    target_list = []
    define_subroutine_set = set()

    # 内部サブルーチン
    for item in TokenSheet2:
        if len(item) <= 7:
            continue
        # if item[6] == "DEFINE" and item[7] == "SUBROUTINE":
        if item[6] == "DEFINE":
            if item[7] == "SUBROUTINE":
                define_subroutine_set.add(item[8])
            else:
                define_subroutine_set.add(item[7])

    for item in TokenSheet2:
        if item[6] != "PERFORM":
            target_list.append(item)
        else:
            if item[7] in define_subroutine_set:
                continue
            target_list.append(item)

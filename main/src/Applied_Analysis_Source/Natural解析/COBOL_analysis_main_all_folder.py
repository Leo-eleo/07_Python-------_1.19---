...
def cobol_connect_accdb(db_path):
...
def main_all_folder(db_base_path, Folder_COBOL_path, COBOL_setting_path,
                    title):
    ...
    COBOL_Folders = glob_files(Folder_COBOL_path,
                               recursive=False,
                               type="folder")
    ...
    for COBOL_Folder in COBOL_Folders:
        ...
        ld = len(COBOL_Files)
        conn = cobol_connect_accdb(db_path)
        cursor = conn.cursor()
        ...
        for i, COBOL_File in enumerate(COBOL_Files):
            try:
                ...
                analysis1_5_structure_COBOL(TokenSheet2, ファイル名,
                                            db_path, 設定条件HIT情報出力, 分析条件HIT情報出力,
                                            設計条件HIT情報出力, 設定条件HIT_NG情報出力, conn,
                                            cursor)
                if COBOL入出力情報出力:
                ...
                true_file_path = os.path.join(COBOL_Folder, os.path.basename(COBOL_File))
                shutil.move(true_file_path, error_path)
                print(f" Error!")
                with open(os.path.join(error_path, "00_Error_Info.txt"), "a", encoding="CP932") as ft:
                    ft.write(f"file_name: {os.path.basename(COBOL_File)}\n")

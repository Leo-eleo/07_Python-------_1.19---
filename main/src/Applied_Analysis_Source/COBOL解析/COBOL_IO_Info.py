...
class SUB_SQL生成_COBOL_2:
    ...
    def insert(self):
        ...
        key_list = ["資産ID","COBOL_ID","FILE_ID","RECORD_ID","COPY","RECORD_SIZE","FDSD_INFO","LINE_INFO" ]
        value_list = [ファイル名,COBOL_ID,FDSD_ID,RECORD_ID,COPY,RECORD_SIZE,FDSD_INFO,ITEM_INFO ]
...
class SUB_SQL生成_COBOL_5:
    ...
    def insert(self):
        ...
        key_list = ["資産ID","COBOL_ID","FILE_ID","RECORD_ID","COPY","RECORD_SIZE","FDSD_INFO","LINE_INFO"]
        value_list = [ファイル名,COBOL_ID,FDSD_ID,RECORD_ID,COPY,RECORD_SIZE_STR,FDSD_INFO,ITEM_INFO ]
    ...
    for i in range(len(TokenSheet2)):
        ...
        if COBOL階層情報 == "環境部":
            if CMD_fld == "SELECT":
                ...
                if len(TokenSheet2_GYO) > 7 and  TokenSheet2_GYO[7] == "OPTIONAL":
                ...
                while 検索列 < len(TokenSheet2_GYO):
                    code_val = TokenSheet2_GYO[検索列]
                    if code_val == "ASSIGN":
                        if 検索列 + 1 < len(TokenSheet2_GYO) and TokenSheet2_GYO[検索列 + 1] == "TO":
            ...
            if CMD_fld == "FD" or CMD_fld == "SD":
                ...
                while 検索行 < len(TokenSheet2):
                    ...
                    # '検索行文字列作成
                    検索行文字列 = 検索行文字列生成処理(TokenSheet2_temp[基準列:]) #   '処理開始列（5列目を引数にする）
                    if ITEM_INFO == "":
                        ITEM_INFO = 検索行文字列
                    else:
                        ITEM_INFO = ITEM_INFO + "\n" + 検索行文字列
                    ...
                    if 検索行 >= len(TokenSheet2):
                        break
                    if TokenSheet2[検索行][基準列] != "COPY" or IsNumeric(TokenSheet2[検索行][基準列]) == False:
                        break
            ...
            if CMD_fld == "OPEN":
                ...
                while 検索列 < len(TokenSheet2_GYO):
                    ...
                    if io == "INPUT":
                        ...
                        SUB_SQL生成_共通_3_.insert("COBOL", ライブラリID, COBOL_ID, PGM_IO, TokenSheet2_GYO[検索列])
                    ...
                    検索列 = 検索列 + 1
                    if 検索列 >= len(TokenSheet2_GYO) or TokenSheet2_GYO[検索列] == ".":
                        break
            
            # 'レイアウトグルーピング
            if CMD_fld == "READ" and len(TokenSheet2_GYO) > 9 and TokenSheet2_GYO[8] == "INTO":
            ...
            elif CMD_fld == "WRITE" and len(TokenSheet2_GYO) > 9 and TokenSheet2_GYO[8] == "FROM":
                ...
                while 検索列 < len(TokenSheet2_GYO):
                    ...
                    検索列 = 検索列 + 1
                    if 検索列 >= len(TokenSheet2_GYO) or TokenSheet2_GYO[検索列] == "." or hit_flg == True:
                        break

    ...
    with open(Filename, errors="ignore") as TS:
        ...
        for strREC in TS:
            ...
            if " TALLYING " in strREC:
                # print(
                #     f"{file} で TALLYING を含む行がありました。現在は無限ループに入ってしまう可能性があるため、解析対象からスキップします。"
                # )
                GYO = GYO + 1
                print(f"Analysis Info    : Skip Line: {GYO} , Reason: TALLYING, File = {file}")
                continue
            if " STRING " in strREC:
                # print(f"\nCobol name: {file} で 行にSTRINGが含まれており、正規表現の遡及処理に時間がかかりすぎる")
                GYO = GYO + 1
                print(f"Analysis Info    : Skip Line: {GYO} , Reason: STRING, File = {file}")
                continue

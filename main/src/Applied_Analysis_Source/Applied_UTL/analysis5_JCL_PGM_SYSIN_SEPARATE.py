...
#'20240215 UPD qian.e.wang
    sql =   """\
            SELECT * FROM 顧客別_JCL_PGM_DSN WHERE SYSIN_PGM <> '' AND PGM_NAME IN ( 'UTACH' , 'ADM' , 'JYAADP' , 'ADARUN3V' )
            """
#'UPD END
...
#'20240215 UPD qian.e.wang
    sql = """\
            DELETE * FROM 顧客別_JCL_PGM_DSN WHERE SYSIN_PGM <> '' AND PGM_NAME IN ( 'UTACH' , 'ADM' , 'JYAADP' , 'ADARUN3V' )
            """
#'UPD END
    cursor.execute(sql)
    conn.commit()

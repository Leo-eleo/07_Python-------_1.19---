...
class スキーマ_基本情報:
    ...
    def insert(self,err_info):
        ...
        value_list = [レイアウト名,COPY_レコード名,COPY_レコード長,マルチレイアウト判定,REDIFINE_有無,X項目のみ,レイアウト行数,err_info+l_err]
...
# '       
    ...
    if parm_val == "変数_タイプ①":
        ...
        COPY_桁数 += len(code_val.replace("V","").replace("S","").replace("+","").replace("-",""))-1
    ...
    # '↓追加　20150714　Takei
        ...
        if code_val2 == "(":
            ...
            COPY_桁数 += len(code_val.replace("V","").replace("+","").replace("-",""))
        ...
        code_val = code_val.replace("V","").replace("+","").replace("-","")
    ...
    # '↓追加　20140708　Takei
        ...
        COPY_桁数 = COPY_桁数 + len(code_val.replace("V", ""))
    # '↓追加　20150714　Takei
    elif parm_val == "VALUE":
        VALUE有無 = "●"
    else:
...
def 設定値一致チェック(parm_val,code_val):
    ...
    # '補足タイプ
        ...
        code_val.replace("P", "").replace("9", "").replace("V", "") == "":
        return True
...
def structure_Scm(TokenSheet2,ScmSheet,レイアウト, ログ解析有 = True,conn=None, cursor=None,レイアウト_len=0):
    ...
    global レイアウト解析情報_
    global depending_on_flag,indexed_by_flag,g0byte_flag

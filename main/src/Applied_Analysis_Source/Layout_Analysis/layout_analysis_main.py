...
def main(db_path, Folder_Scm_path, Scm_setting_path,IsDelete=True):

    print("start preparation for analysis.")

    title = "C:\\Users\\kohei.mori\\Documents\\言語解析\\schema_status"
    
    ScmSheet = pd.read_excel(Scm_setting_path,sheet_name="COPY句設定シート")
    ...
    for i,Scm_File in enumerate(Scm_Files):
        ...
        AnalyzeSheet = structure_Scm(TokenSheet2,ScmSheet,ファイル名,True,conn,cursor,レイアウト行数)

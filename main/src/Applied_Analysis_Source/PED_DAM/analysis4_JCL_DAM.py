...
class 顧客別_JCL_PED_DAMデータセット:
    ...
    def insert(self,assetID,jobSeq ,jclId,stepSeq ,stepName,pgmName,procName,ddName,cmdSeq ,pedName):
        ...
        if pedName in self.ped_dam_dic:
            ...
            for data in df:
                ActSheet_x = [""]*16
                key_list = ["資産ID","JOB_SEQ","JCL_ID","STEP_SEQ","STEP_NAME","PGM_NAME","PROC_NAME","DD_NAME","CMD_SEQ","PED_NAME","SCHEMA_NAME","DAM_DD_NAME","DATASET_NAME","ACCESS_MODE"]
                value_list = [assetID,jobSeq,jclId,stepSeq,stepName,pgmName,procName,ddName,cmdSeq,pedName,data["SCHEMANAME"],data["DDNAME"],data["DSN"],data["ACCESSMODE"]]
...
def analysis4_JCL_DAM(conn,cursor):
    global 顧客別_JCL_PED_DAMデータセット_ 
    global ActSheet

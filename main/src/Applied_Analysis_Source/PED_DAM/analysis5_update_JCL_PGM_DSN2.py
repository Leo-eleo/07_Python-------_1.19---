...
def analysis5_update_JCL_PGM_DSN2(conn,cursor):
    global JCL_PGM_DSN_
    
    sql = "SELECT * FROM 顧客別_JCL_PGM_DSN WHERE 自動更新FLG = '自動設定(DAM解析)'"
    ...
    sql =   """\
            SELECT * FROM 顧客別JCL_PED_DAMDATASET WHERE 備考 <> 'PED定義未受領'
            """
    ...
    for i in range(len(df)):
        ...
        JCL_PGM_DSN_.insert(library, data["資産ID"], data["JOB_SEQ"], jobId, \
                               ...
                               "", data["DAM_DD_NAME"], data["DATASET_NAME"], data["備考"], DB自動登録時PARM)

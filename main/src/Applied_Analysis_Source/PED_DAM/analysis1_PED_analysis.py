...
class PED解析結果出力処理:
    ...
    def insert(self,pedFile, pedName, schemaName, ddName, accessMode,dsnList):
        if self.dic == None:
            self.setup()
        ...
        for dsn in dsnList:
            ...
            key_list = ["PEDFILENAME","LIBRARY","GOUKI","MODULEID","PEDNAME","SCHEMANAME","DDNAME","ACCESSMODE","DSN"]
            value_list = [FileName,FileLibrary,FileGouki,FlieModule,pedName, schemaName, ddName, accessMode,dsn]
    ...
    with open(ped_file,errors="ignore") as tf:
        ...
        for line in data1:
            ...
            if "PED" in line:
                ...
                PED解析結果出力処理_.insert(pedFile, pedName, schemaName, ddName, accessMode,GetDSN_.get(schemaName))
                schemaName = ""
                ddName = ""
                accessMode = ""
...
def analysis1_PED_analysis(conn,cursor,ped_file_path):
    global PED解析結果出力処理_,GetDSN_

...
def replace_line(line):
  ...
  # 正規表現パターン
  pattern = r"1\s+PAGE\s+\d{4}\s+\n"

  # 正規表現で置換
  replaced_line = re.sub(pattern, "", line)
...
def assetDivisionHandler(sourcePath, resultPath):
    ...
    for file in sources:
        ...
        with open(file, "r", errors="ignore") as ft:
            content = [tmp for tmp in ft.read().split("MEMBER NAME") if tmp != ""]
            for mn in content:
                mn = replace_line(mn)
                endIndex = mn.find("\n") + 1
                ...
                if mn.startswith("V") or mn.startswith(" "):
                    mn = mn[1::]
                
                # patternFile が "" 以外の場合のみ出力
                if patternFile != "":
